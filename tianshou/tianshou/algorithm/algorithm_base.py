...
# [学习导读-核心抽象] Policy 是“行动接口”：它把一批 observation 映射为一批动作。
# 它不决定如何计算 loss；这样同一 Policy 可以被不同 Algorithm、Collector 和评估代码复用。
class Policy(nn.Module, ABC):
    ...
    def __init__(
        ...
        """
        # [学习导读-核心抽象] 这里保存的是 policy 与 Gymnasium 环境之间的契约。
        # action_space 决定动作是离散还是连续；连续动作才可能需要 clip/tanh 和缩放。
    ...
    def map_action(
        ...
        :return: action in the same form of input "act" but remap to the target action
            space.
        """
        # [学习导读-核心抽象] 网络输出是“学习用的原始动作”；环境动作空间才是“执行用的动作”。
        # 对离散动作通常不变，对连续动作则在此处做边界约束与缩放。
    ...
    def compute_action(
        ...
        """
        # [学习导读-核心抽象] 这是单环境推理的便利入口：手动补上 batch 维，
        # 调用统一的 forward，再把结果还原为环境可直接接受的单个动作。
    ...
    def add_exploration_noise(
        ...
        """
        # 基类不探索，具体策略可覆写：DQN 用 epsilon-greedy，连续控制常加高斯/OU 噪声。
        return act
...
# [学习导读-核心抽象] Algorithm 是“学习接口”：它持有 Policy，并定义如何把 buffer 中的
# Batch 变成 loss、梯度和新参数。把行动与学习分开，是 Tianshou v2 最重要的工程边界之一。
class Algorithm(torch.nn.Module, Generic[TPolicy, TTrainerParams], ABC):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        # [学习导读-核心抽象] Algorithm 不直接保存环境；它只保存可学习的模型、优化器和调度器。
        # 环境交互由 Collector 负责，训练节奏由 Trainer 负责。
    ...
    def _preprocess_batch(
        ...
        """
        # [学习导读-核心抽象] 原始 transition 还不一定能直接算 loss。
        # 子类在这里添加 returns、advantage、target Q 等“由轨迹关系推导出的训练字段”。
        return batch
    ...
    def _update(
        ...
        """
        # [学习导读-核心抽象] 这是所有算法共享的更新骨架：
        # sample Batch -> 预处理 -> 子类计算 loss/反向传播 -> 后处理 -> 学习率调度。
        # 具体 DQN/PPO/SAC 的差异被限制在 update_with_batch_fn 中。
    ...
    def run_training(self, params: TTrainerParams) -> "InfoStats":
        # [学习导读-核心抽象] Algorithm 知道自己应匹配哪种 Trainer，
        # 但不把 epoch、日志和测试循环硬编码进具体算法实现。
