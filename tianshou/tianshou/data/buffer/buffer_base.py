...
# [学习导读-阶段2] ReplayBuffer 是带 episode 边界信息的环形 Batch：容量满后覆盖最旧经验。
# 它把“采样数据”和“时间相邻关系”同时保存下来，供 DQN 的随机采样和 n-step return 使用。
class ReplayBuffer:
    ...
    def __init__(
        ...
        **kwargs: Any,  # otherwise PrioritizedVectorReplayBuffer will cause TypeError
        ...
        # [学习导读-核心抽象] Buffer 只关心“怎样可靠地保存和取出经验”，
        # 不关心 DQN/PPO 的 loss。`_meta` 是真正存储各字段数组的底层 Batch。
    ...
    def next(self, index: int | np.ndarray) -> np.ndarray:
        ...
        """
        # [学习导读-阶段2] 不能跨 episode 取 next；若当前 transition 已结束，
        # 返回自身，避免把下一条新 episode 的 obs 错当作当前轨迹的后继状态。
        end_flag = self.done[index] | (index == self.last_index[0])
    ...
    def add(
        ...
        episode_reward is 0.
        """
        # [学习导读-阶段2] `Collector` 每一步交来一组 transition。这里先复制，
        # 再统一计算 done = terminated OR truncated，避免修改调用者持有的 batch。
        ...
        # 更新环形写指针和本 episode 的累计回报/长度；写满后 insertion_idx 会回到 0。
    ...
    def sample_indices(self, batch_size: int | None) -> np.ndarray:
        ...
        :param batch_size: the number of indices to be sampled. If None, it will be set
            ...
            random order).
        """
        # [学习导读-阶段2] 先抽索引、再用这些索引取 Batch。把二者分开使 PER 等子类
        # 可以只替换“怎样抽样”，而不用重复“怎样读取数据”。
    ...
    def sample(self, batch_size: int | None) -> tuple[RolloutBatchProtocol, np.ndarray]:
        ...
        """
        # 返回 indices 是为了让算法在 PER 等场景中把 TD error 写回对应经验。
