...
# [学习导读-阶段2] Batch 是 Tianshou 的统一数据语言：一次 transition、一批 transition、
# policy 输出和训练样本都用它承载。约定每个数组/张量的第 0 维是 batch 维。
# [学习导读-核心抽象] Collector 用它打包环境交互，ReplayBuffer 用它存储与采样，
# Algorithm 用它读取训练字段；它是各模块之间避免长参数列表的数据契约。
class Batch(BatchProtocol):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # [学习导读-阶段2] 构造时会递归解析 dict、数组、张量和嵌套 Batch，
        # 让后续的切片、拼接和设备转换可以对所有字段保持一致。
        if copy:
    ...
    def __getitem__(self, index: str | IndexType) -> Any:
        ...
        # [学习导读-阶段2] 数值/slice 索引不是只切某个字段，而是同时切所有字段，
        # 因而 `batch[:32]` 仍是一组对齐的 (obs, act, rew, ...) transition。
    ...
    def to_torch(
        ...
        # [学习导读-阶段2] 这是“返回副本”的转换；collector/buffer 中通常保存 NumPy，
        # 算法更新前才把采样 batch 转为指定 device 上的 torch tensor。
        result = deepcopy(self)
