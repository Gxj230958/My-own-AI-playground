...
# [学习导读-核心抽象] Collector 是“交互接口”：它反复调用 Policy、推进环境、
# 把结果组织成 transition 并交给 ReplayBuffer。它不计算强化学习 loss。
class BaseCollector(Generic[TCollectStats], ABC):
    ...
    def __init__(
        ...
        """
        # [学习导读-核心抽象] 传入 Algorithm 时只取其中的 policy；这说明采样阶段只需要
        # “如何行动”，而不需要知道“如何学习”。buffer=None 时创建默认 buffer 仅是易用性兜底。
    ...
    def collect(
        ...
        """
        # [学习导读-阶段3] 这是采样的公共入口。n_step 适合按“数据量”训练，
        # n_episode 适合按“完整轨迹数”评估；一次调用只能选择其中之一。
...
class Collector(BaseCollector[TCollectStats], Generic[TCollectStats]):
    ...
    def _compute_action_policy_hidden(
        ...
        # [学习导读-阶段3] 这一层把环境的 NumPy observation 包装为 Batch，交给 policy；
        # 然后把网络原始输出映射为 action space 可接受的动作。RNN 的 hidden state 也在此传递。
        if random:
            try:
    ...
    def _collect(  # noqa: C901
        ...
        """
        # [学习导读-阶段3] 不要试图第一次就读完这个函数；只沿 Step 2 -> 3 -> 6
        # 跟一次数据流：policy 出动作、env 返回结果、buffer 写入 transition。
        ...
        while True:
            ...
            # [学习导读-阶段3] `current_step_batch_R` 就是一批完整 transition：
            # (obs, act, rew, obs_next, terminated, truncated, info)。复制后写入 buffer，
            # 以免后续 collector 更新临时状态时影响已经保存的经验。
