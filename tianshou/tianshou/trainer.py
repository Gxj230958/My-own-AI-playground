...
# [学习导读-核心抽象] Trainer 是“流程编排接口”：它不实现 DQN/PPO 的公式，
# 而是决定何时 collect、update、test、记录日志、保存最佳模型和停止训练。
class Trainer(Generic[TAlgorithm, TTrainerParams], ABC):
    ...
    def __init__(
        ...
        # [学习导读-核心抽象] Trainer 把“算法本体”与“本次实验配置”绑定为一次可运行训练。
        # 下面的计数器与 best score 都是实验状态，不属于 Policy 或 Algorithm 的数学定义。
    ...
    def execute_epoch(self) -> EpochStats:
        # [学习导读-核心抽象] 一个 epoch 是训练管理单位，不等于一个 episode。
        # 本方法循环调用子类 _training_step，再统一处理日志、checkpoint 和测试。
        self._epoch += 1
    ...
    def run(
        ...
        :param reset_collector_buffers: whether, for the case where the collectors are reset, to reset/clear the
            ...
            This has no effect if `reset_collectors` is False.
        """
        # [学习导读-核心抽象] run 是最高层入口：先 reset，再反复执行 epoch，
        # 直到达到 max_epochs 或测试指标触发提前停止。
        self.reset(
