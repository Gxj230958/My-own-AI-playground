...
# Part A / Part B 新手提示：
# 1. 训练 MLP 时，数据保持当前形状 [N, 28 * 28]，也就是 [N, 784]。
# 2. 训练 CNN 时，需要把数据 reshape 成 [N, 1, 28, 28]，
#    因为 conv2D.forward 期望输入是 [batch, channels, H, W]。
# 3. 做 MLP 和 CNN 对比时，尽量保持训练/验证划分、归一化方式、epoch、
#    batch size、loss 和 metric 一致，这样报告里的比较更公平。
...
# 注意：如果 MultiStepLR 还没有实现，先把 scheduler 设为 None，
# 或者临时使用已经实现的 StepLR。否则 Part A 的 MLP 还没验证就可能先报错。
