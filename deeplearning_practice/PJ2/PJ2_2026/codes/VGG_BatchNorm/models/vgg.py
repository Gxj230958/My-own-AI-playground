...
except ImportError:  # 允许从仓库根目录以 package 形式导入本文件。
    from ..utils.nn import init_weights_
...
class VGG_A_BatchNorm(nn.Module):
    """VGG-A with Batch Normalization for CIFAR-10.

    结构与 VGG_A 保持一致，只是在每个卷积层后面加入 BatchNorm2d，
    在前两个隐藏全连接层后面加入 BatchNorm1d。这样做可以尽量保证
    “是否使用 BN” 是主要变量，便于和原始 VGG_A 做公平对比。
    """
    ...
    def __init__(self, inp_ch=3, num_classes=10, init_weights=True):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(512 * 1 * 1, 512, bias=False),
            nn.BatchNorm1d(512),
            nn.ReLU(True),
